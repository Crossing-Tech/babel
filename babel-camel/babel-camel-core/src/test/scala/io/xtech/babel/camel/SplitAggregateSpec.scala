...
class SplitAggregateSpec extends SpecificationWithJUnit {
  ...
  "create a route with an aggregate eip the camel way" in new camel {
    ...
    val routeDef = new RouteBuilder {
      ...
      from("direct:babel").as[String].splitBody(_.split(",").iterator).to("mock:babel1").requireAs[String].processBody(_.toInt + 1 + "").to("mock:babel2").requireAs[String].
